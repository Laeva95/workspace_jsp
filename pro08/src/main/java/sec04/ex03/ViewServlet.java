	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			out.print("<tr><td>" + vo.getId() + "</td><td>"
								...
								+ "<a href='/pro08/member4?command=delMember&id=" + vo.getId() + "' onclick=\"return confirm('정말 삭제하시겠습니까?');\">삭제</a></td><td>"
								+ "<a href='/pro08/member4?command=modMember&id=" + vo.getId() + "'>수정</a></td>");
		}
		out.print("</table>");
